...
#         simplified_translation = tokenizer.batch_decode(generated_tokens, skip_special_tokens=True)[0]
#         return converter.convert(simplified_translation)
...
#                                 logging.info(f"Translated [{index+1}/{len(lines)}]: {text_id} -> {translated_text}")
...
# output_path = "/share/nas169/jerryyang/corpus/librispeech_to_mandarin"  # 替換為實際輸出路徑

# datasets = ["train-clean-100", "train-clean-360", "train-other-500"]
# for dataset in datasets:
#     process_directory(os.path.join(librispeech_path, dataset), os.path.join(output_path, dataset))
...
librispeech_path = "/share/nas169/jerryyang/corpus/LibriSpeech/LibriSpeech"  # 替換為實際路徑
output_path = "/share/nas169/jerryyang/corpus/M2M100"  # 替換為實際輸出路徑
