...
def prepare_mandarin(
    ...
    # 這個不用的話應該可以註解掉，我隨便改一下而已，沒有全部檢查
    # Additional checks to make sure the data folder contains mandarin
    ...
    for split_index in range(len(splits)):
        ...
        text_lst = get_all_files(
            os.path.join(data_folder, "text"), match_and=["new.txt"]
        )
        ...
        if select_n_sentences is not None:
            # 這行可能有問題，因為select_n_sentences是整數，不過應該用不到這行所以沒差
...
def check_mandarin_folders(data_folder, splits):
    """
    ...
    ------
    OSError
        If mandarin is not found at the specified path.
    """
